...
export function MorrowHome(){
  const[showNote,setShowNote]=useState(false);const[mode,setMode]=useState<Experience>("Write");
  const[format,setFormat]=useState<Format>("Novel");const[language,setLanguage]=useState<Language>("English");
  const[googleStep,setGoogleStep]=useState(0);const[revisionAdded,setRevisionAdded]=useState(false);
  ...
  return <main className={styles.home}>
    <section className={styles.hero}>
      ...
      <div className={styles.heroArt}><Image src="/images/morrow-story-world.png" fill priority sizes="(max-width: 900px) 100vw, 60vw" alt="An illustrated manuscript on a writer's desk with a silver key and a story world emerging from its pages"/><div className={styles.editorFloat}><span>Chapter 8 · The conservation room</span><p>She handed him <button onClick={()=>setShowNote(true)}>the silver key</button> and watched the door close between them.</p>{showNote&&<aside><b>A detail to revisit</b><p>Lena still has the key in Chapter 8.</p><button onClick={()=>setShowNote(false)}>See the earlier scene →</button></aside>}</div></div>
    </section>
    ...
    <section className={styles.memory}><div><p className={styles.overline}>Intelligence without interruption</p><h2>Your story remembers the rest.</h2><p>Who knows the secret. Where the key was left. Which promise still needs an ending. Change one detail and Morrow gently shows you where else to look.</p></div><div className={styles.memoryScene}><span>Chapter 3</span><p>Lena hides the key inside the blue frame.</p><i/><span>Chapter 8</span><p>“I gave it to Tomas,” she says.</p><aside><b>Worth another look</b>These moments may not agree. You decide which one changes.</aside></div></section>
