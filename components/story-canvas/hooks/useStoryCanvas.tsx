...
type Context = {
  ...
  saveStatus: "Editing" | "Saved locally" | "Saving to Morrow" | "Saved to Morrow" | "Offline" | "Retry required" | "Revision conflict"; retrySave: () => void; notice: string; setNotice: (value: string) => void; dataSource: StoryWorkspaceDataSource;
...
export function StoryCanvasProvider({ children, projectId = "museum-of-lost-hours", mode = "demo", initialState }: { children: ReactNode; projectId?: string; mode?: "demo" | "production"; initialState?: CanvasState }) {
  ...
  const source = useMemo<StoryWorkspaceDataSource>(() => mode === "production" ? new ProductionStoryDataSource(projectId, initialState) : new LocalDemoStoryDataSource(), [initialState, mode, projectId]);
  const [saveStatus, setSaveStatus] = useState<Context["saveStatus"]>(mode === "demo" ? "Saved locally" : "Saved to Morrow");
  ...
  useEffect(() => { if (initialState) return; let active = true; source.loadProject(projectId).then((restored) => { if (active) rawDispatch({ type: "RESTORE", state: { ...restored, dataMode: mode } }); }).catch(() => setSaveStatus(navigator.onLine ? "Retry required" : "Offline")).finally(() => setHydrated(true)); return () => { active = false; }; }, [initialState, mode, projectId, source]);
  useEffect(() => { if (!hydrated) return; if (mode === "demo") setSaveStatus(navigator.onLine ? "Editing" : "Offline"); if (timer.current) clearTimeout(timer.current); timer.current = setTimeout(async () => { try { await writeIndexedCanvasState(state); if (mode === "demo") setSaveStatus(navigator.onLine ? "Saved locally" : "Offline"); } catch { setSaveStatus("Retry required"); } }, 450); return () => { if (timer.current) clearTimeout(timer.current); }; }, [state, hydrated, mode]);
  useEffect(() => { if (!hydrated || mode !== "production") return; const scene = state.scenes.find((item) => item.id === state.currentSceneId); if (!scene || (retryNonce === 0 && lastServerText.current.get(scene.id) === scene.manuscriptText)) return; if (!lastServerText.current.has(scene.id) && retryNonce === 0) { lastServerText.current.set(scene.id, scene.manuscriptText); return; } if (serverTimer.current) clearTimeout(serverTimer.current); setSaveStatus(navigator.onLine ? "Saving to Morrow" : "Offline"); serverTimer.current = setTimeout(async () => { try { const saved = await source.saveScene(scene); lastServerText.current.set(scene.id, scene.manuscriptText); rawDispatch({ type: "SCENE_SAVED", sceneId: scene.id, revision: saved.revision }); setSaveStatus("Saved to Morrow"); } catch (error) { const status = typeof error === "object" && error && "status" in error ? Number(error.status) : 0; setSaveStatus(!navigator.onLine ? "Offline" : status === 409 ? "Revision conflict" : "Retry required"); setNotice(error instanceof Error ? `${error.message} · Retry required` : "The manuscript could not be saved · Retry required"); } }, 700); return () => { if (serverTimer.current) clearTimeout(serverTimer.current); }; }, [hydrated, mode, retryNonce, source, state.currentSceneId, state.scenes]);
  ...
  const value = useMemo(() => ({ state, dispatch, undo, canUndo: history.current.length > 0, saveStatus, retrySave, notice, setNotice, dataSource: source, createPart, createChapter, createScene, createEntity, structure }), [state, dispatch, undo, saveStatus, retrySave, notice, source, createPart, createChapter, createScene, createEntity, structure]);
