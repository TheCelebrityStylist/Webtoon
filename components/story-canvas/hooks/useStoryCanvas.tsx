...
export function StoryCanvasProvider({ children, projectId = "museum-of-lost-hours", mode = "demo", initialState }: { children: ReactNode; projectId?: string; mode?: "demo" | "production"; initialState?: CanvasState }) {
  ...
  useEffect(() => { if (!hydrated || mode !== "production") return; const scene = state.scenes.find((item) => item.id === state.currentSceneId); if (!scene || lastServerText.current.get(scene.id) === scene.manuscriptText) return; if (!lastServerText.current.has(scene.id)) { lastServerText.current.set(scene.id, scene.manuscriptText); return; } if (serverTimer.current) clearTimeout(serverTimer.current); setSaveStatus(navigator.onLine ? "Saving" : "Offline"); serverTimer.current = setTimeout(async () => { try { const saved = await source.saveScene(scene); lastServerText.current.set(scene.id, scene.manuscriptText); rawDispatch({ type: "SCENE_SAVED", sceneId: scene.id, revision: saved.revision }); setSaveStatus("Saved"); } catch (error) { setSaveStatus(navigator.onLine ? "Conflict" : "Offline"); setNotice(error instanceof Error ? error.message : "The manuscript could not be saved"); } }, 700); return () => { if (serverTimer.current) clearTimeout(serverTimer.current); }; }, [hydrated, mode, source, state.currentSceneId, state.scenes]);
