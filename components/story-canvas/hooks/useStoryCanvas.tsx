...
export function StoryCanvasProvider({ children, projectId = "museum-of-lost-hours", mode = "demo", initialState }: { children: ReactNode; projectId?: string; mode?: "demo" | "production"; initialState?: CanvasState }) {
  ...
  useEffect(() => { if (initialState) return; let active = true; source.loadProject(projectId).then((restored) => { if (!active) return; const requested = new URLSearchParams(window.location.search).get("mode"); const restoredMode = requested && ["write", "world", "branches"].includes(requested) ? requested as CanvasState["mode"] : restored.mode; rawDispatch({ type: "RESTORE", state: { ...restored, mode: restoredMode, dataMode: mode } }); }).catch(() => setSaveStatus(navigator.onLine ? "Conflict" : "Offline")).finally(() => setHydrated(true)); return () => { active = false; }; }, [initialState, mode, projectId, source]);
