...
export function WorkspaceSync({ projectId, sync, demo, onClose, onImport, onSyncState }: { projectId: string; sync: GoogleWorkspaceState; demo: boolean; onClose: () => void; onImport: (preview: GoogleImportPreview) => void; onSyncState: (next: Partial<GoogleWorkspaceState>) => void }) {
  ...
  const link = async (action: "create" | "link") => { const result = await request<{ documentId: string; documentName: string; documentUrl: string; lastSyncedAt?: string; status: "synced" | "connected" }>(`/api/projects/${projectId}/google/link`, { action, documentId: action === "link" ? documentId : undefined }); onSyncState({ ...result, status: result.status }); setInspection(null); setView("home"); };
  ...
  return <div className="canvas-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className="sync-dialog" role="dialog" aria-modal="true" aria-label="Workspace Sync">
    ...
    {view === "picker" && <div className="picker-view"><button className="back" onClick={() => { setInspection(null); setView("home"); }}>Back</button><StoryIcon name="drive" /><h3>Choose a Google Doc</h3><p>Open the secure Google Picker. Selecting and inspecting a Doc never changes it.</p><button className="google-picker" onClick={() => window.dispatchEvent(new CustomEvent("morrow:google-picker"))}>Open Google Picker</button><label>Selected document<input value={documentId} onChange={(event) => { setDocumentId(event.target.value); setInspection(null); }} placeholder="Choose a Doc or paste its ID" /></label>{inspection ? <section className="link-inspection"><h4>{inspection.title}</h4><p>{inspection.empty ? "Empty document" : `${inspection.wordCount} words`} · {inspection.headings} headings · {inspection.namedRangeCount} named ranges</p><small>Linking stores this document reference only. Morrow will not replace or edit its content.</small><div className="picker-actions"><button onClick={() => void loadPreview()}>Import with preview</button><button className="primary" disabled={busy} onClick={() => void link("link")}>Link without changing Doc</button><button onClick={() => void link("create")}>Create a new Morrow copy</button><button onClick={() => { setInspection(null); setDocumentId(""); }}>Cancel</button></div></section> : <div className="picker-actions"><button disabled={documentId.length < 10 || busy} onClick={() => void loadPreview()}>{busy ? "Reading…" : "Preview import"}</button><button className="primary" disabled={documentId.length < 10 || busy} onClick={() => void inspectLink()}>Review link choices</button></div>}</div>}
