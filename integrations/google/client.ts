...
export async function googleJson<T>(userId:string,scope:string,url:string,init?:RequestInit){const token=await googleAccessToken(userId,scope);const response=await fetch(url,{...init,headers:{authorization:`Bearer ${token}`,"content-type":"application/json",...(init?.headers??{})},cache:"no-store"});if(!response.ok)throw new GoogleConnectionError(`Google request failed (${response.status})`,response.status);return response.json() as Promise<T>}
export class GoogleConnectionError extends Error{constructor(message:string,readonly status:number){super(message)}}
export function googleErrorResponse(error:unknown){if(error instanceof GoogleConnectionError)return Response.json({error:error.message,code:"GOOGLE_PROVIDER_ERROR"},{status:error.status>=500?502:error.status});return Response.json({error:"Google request failed",code:"GOOGLE_PROVIDER_ERROR"},{status:502})}
