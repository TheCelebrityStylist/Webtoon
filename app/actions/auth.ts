...
const signUpInput = z.object({ name: z.string().trim().min(2).max(80), email: z.string().email(), password: z.string().min(12).regex(/[A-Z]/).regex(/[0-9]/) });
export async function signUp(_: FormState, formData: FormData): Promise<FormState> {
  ...
  await prisma.user.create({ data: { email, passwordHash: await hash(parsed.data.password, 12), profile: { create: { displayName: parsed.data.name } }, memberships: { create: { role: "OWNER", workspace: { create: { name: `${parsed.data.name}'s studio` } } } } } });
